...
@Component({
  selector: 'Deployment-Table',
  templateUrl: './deploymentMain.component.html',
  //styleUrls: ['./mediaStoreTable.component.scss'],
  providers:	[CollectionTableDataCollectorService],
  encapsulation: ViewEncapsulation.None
})
export class deploymentMainTable implements OnInit {

    tableData :any=[];
    columns= [
      {
        key: 'mediaProfile',
        label: 'Media Profile Name'
      },
      {
        key: 'mediaAssetGroup',
        label: 'Media Asset Group'
      },
      /*{
        key:"Info",
        label:"Info",
        fn:function(value,object){
          var retStr=""
          let overflowCnt=0;
          /*for(let i=0;i<value.length;i++,overflowCnt++)
          {
            if(overflowCnt>2)
            {
              retStr=retStr+"<b>...</b>";
            ...
            retStr= retStr+value[i].filename+",<br>";
          }***
          return retStr;
          }
      },*/
...
, vcRef: ViewContainerRef) {
  ...
  ngOnInit() {
      ...
      this.namespace = this.nsObj.getNameSpace("MediaDeployment");
      this.getAllAssetProfiles();
      this.getAllMediaProfies();
...
getAllAssetProfiles(){
  let ns = this.nsObj.getNameSpace("MediaAssetGroup");
  try{
    var paramsForData1={};
    let query={};
    let projectQuery={};
      ...
      this.tableDataCollectService.getPostData(paramsForData1)
        .subscribe(result => {
              ...
              this.allMediaFiles.forEach(element => {
                console.log(element);
                deviceNames.push(element["profileName"]);
              });
...
getAllMediaProfies(){
  let ns = this.nsObj.getNameSpace("MediaProfiles");
  try{
    var paramsForData1={};
    let query={};
    let projectQuery={};
    projectQuery["profileName"]=1;
    projectQuery["date"]=1;
    projectQuery["uuid"]=1;
    paramsForData1["projectQuery"]=projectQuery;
        paramsForData1["dataQuery"]= query;
    ...
    paramsForData1["limit"]=100;
      this.tableDataCollectService.getPostData(paramsForData1)
        .subscribe(result => {
              ...
              this.allMediaProfiles=result.data;
              console.log(this.allMediaProfiles);

              try{
                let temp=[];

                this.allMediaProfiles.forEach(element => {
                  console.log(element);
                  temp.push(element["profileName"]);
                });
                this.allMediaProfiles=temp;
