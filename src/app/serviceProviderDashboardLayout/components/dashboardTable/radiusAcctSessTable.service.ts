...
export class RadiusTableDataCollectorService{
 ...
 constructor(private http: Http, private url:UrlProvider) { 
  ...
  getData(limit:any){
      ...
      let url="http://localhost:4000/tableData/dataForCollectionTable?namespace="+limit.namespace+"&start="+oldDate+"&current="+currDate;
      if((l>0)&&(String(l)!="")&&(String(l)!="undefined"))
        url = url+"&limit="+String(l);
      else
        url="http://localhost:4000/tableData/dataForCollectionTable?namespace="+limit.namespace+"&start="+oldDate+"&current="+currDate;
      let dat = limit.date;
