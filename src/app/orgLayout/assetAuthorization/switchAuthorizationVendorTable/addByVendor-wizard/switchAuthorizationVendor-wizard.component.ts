...
@Component({
  selector: 'switchAuthorizationVendor-add-wizard',
  templateUrl: './switchAuthorizationVendor-wizard.component.html',
  styleUrls: ['./switchAuthorizationVendor-wizard.component.scss'],
  providers:[RPCService]
})
export class SwitchAuthorizationVendorAddWizardComponent implements OnInit {
...
let locsFullData = this.locationData.filter(val => formData.locations.includes(val.uuid));
  ...
  this.clientRpc.orgRPCCall(op,"orgManagerAuthorisedSwitchesVendorMethod",finalData)
        .subscribe(res =>{
        console.log("OUTPUT...");
        console.log(res);
        let log = new Logger();
        if(String(res.type)=="undefined")
        {
          ...
          if((res.result.status=="Success")||(res.result.Status=="Success"))
          {
            ...
            log.storeSuccessNotification("In Switch Authorization Vendor "+this.operation+" succeeded");
          }
          else
          {
            log.storeFailNotification("In Switch Authorization Vendor "+this.operation+" Failed");
