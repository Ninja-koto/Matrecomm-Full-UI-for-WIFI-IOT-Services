...
this.countries= ['Argentina','Bolivia','Brazil','Chile','Colombia','Ecuador','Guyana','Paraguay','Peru','Suriname', 'Uruguay','Venezuela'];
  ...
  this.clientRpc.serviceProviderRPCCall(op,"productManagerAdminMethod",finalData)
        .subscribe(res =>{
        console.log("OUTPUT...");
        console.log(res);
        let log = new Logger();
        if(String(res.type)=="undefined")
        {
          ...
          if((res.result.status=="Success")||(res.result.Status=="Success"))
          {
            ...
            log.storeSuccessNotification("In ServiceProvider Admin "+this.operation+" succeeded");
          }
          else
          {
            log.storeFailNotification("In ServiceProvider Admin "+this.operation+" Failed");
