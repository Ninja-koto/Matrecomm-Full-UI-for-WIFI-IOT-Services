...
@Component({
  selector: 'alert-TeamProfiles',
  templateUrl: './alertTeamProfilesTable.html',
  styleUrls: ['./alertTeamProfilesTable.scss'],
  providers:	[CollectionTableDataCollectorService],
  encapsulation: ViewEncapsulation.None
})
export class AlertTeamProfilesTable implements OnInit {
    ...
    columns= [
     {
      ...
      },
      {
        ...
        fn:function(value,object){
          let itCount=3;
          var locStr ="";
          let locs = object.users;
          for(let i=0;i<locs.length;i++)
          {
            if(i>itCount) break;
            if(i>0) locStr = locStr+",<br>";
            locStr = locStr+'<b>Name:</b>'+locs[i].firstName+', <b>Mobile:</b>'+locs[i].mobilePhone;
          }
          return locStr;
          }
      },
      {
        ...
        label: 'Actions',
        fn:function(value,object){
          let email = object.emailEvent;
          let sms = object.smsEvent;
          let locStr="";
          if(String(sms)=="true")
          {
            locStr = locStr+"SMS<br>";
          }
          if(String(email)=="true")
          {
            locStr = locStr+"E-Mail";
          }
          return locStr;
          }
      }
...
, vcRef: ViewContainerRef) {
  ...
  ngOnInit() {
     this.nsObj = new NameSpaceUtil(this.storage);
      this.namespace = this.nsObj.getNameSpace("AlertMgrTeam");
      this.getAllAlertMgrUsers();
...
getAllAlertMgrUsers(){
      let ns = this.nsObj.getNameSpace("AlertMgrUser");
      try{
        var paramsForData1={};
        let query={};
        ...
        paramsForData1["limit"]=100;
          this.tableDataCollectService.getPostData(paramsForData1)
            .subscribe(result => {
                  ...
                  this.alertMgrUsers=result.data;
