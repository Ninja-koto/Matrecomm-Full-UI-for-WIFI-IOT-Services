...
@Component({
  selector: 'assetAuthorizationVendor-add-wizard',
  templateUrl: './assetAuthorizationVendor-wizard.component.html',
  styleUrls: ['./assetAuthorizationVendor-wizard.component.scss'],
  providers:[RPCService]
})
export class AssetAuthorizationVendorAddWizardComponent implements OnInit {
...
let profileName = this.myForm.controls.profileName.value;
  ...
  this.clientRpc.orgRPCCall(op,"orgManagerAuthorisedAssetsVendorMethod",finalData)
        .subscribe(res =>{
        console.log("OUTPUT...");
        console.log(res);
        let log = new Logger();
        if(String(res.type)=="undefined")
        {
          ...
          if((res.result.status=="Success")||(res.result.Status=="Success"))
          {
            ...
            log.storeSuccessNotification("In AP Authorization Vendor "+this.operation+" succeeded");
          }
          else
          {
            log.storeFailNotification("In AP Authorization Vendor "+this.operation+" Failed");
