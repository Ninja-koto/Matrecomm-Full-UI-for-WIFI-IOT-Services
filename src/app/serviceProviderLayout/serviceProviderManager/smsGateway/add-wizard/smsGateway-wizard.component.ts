...
export class SMSAddWizardComponent implements OnInit {
  ...
  this.clientRpc.serviceProviderRPCCall(op,"productManagerSmppMethod",finalData)
        .subscribe(res =>{
        console.log("OUTPUT...");
        console.log(res);
        let log = new Logger();
        if(String(res.type)=="undefined")
        {
          ...
          if((res.result.status=="Success")||(res.result.Status=="Success"))
          {
            ...
            log.storeSuccessNotification("In ServiceProvider SMS "+this.operation+" succeeded");
          }
          else
          {
            log.storeFailNotification("In ServiceProvider SMS "+this.operation+" Failed");
