...
@Component({
  selector: 'productMgr-organization',
  templateUrl: './organization.component.html',
  styleUrls: ['./organization.component.scss'],
  providers:	[CollectionTableDataCollectorService],
  encapsulation: ViewEncapsulation.None
})
export class ProductManagerOrganizationComponent implements OnInit {
    tableData :any;
    columns= [
      {
...
getAllDataCenters(){
      let ns = this.nsObj.getNameSpace("DataCenter");
      try{
        var paramsForData1={};
        let query={};
            paramsForData1["dataQuery"]= query;
        paramsForData1["namespace"]= ns;
        paramsForData1["limit"]=1000;
          this.tableDataCollectService.getPostData(paramsForData1)
            .subscribe(result => {
                  console.log("All DataCenters : ");
                  this.allDataCenters=result.data;
                  //console.log(this.allLocations);
          });
      }
      catch(e)
      {
        console.log("getAllDataCenters Data load Failed... ");
      }
}
startInterval(){
  var paramsForData={};
