...
@Component({
  selector: 'vehicleConfiguration-add-wizard',
  templateUrl: './vehicleConfiguration-wizard.component.html',
  styleUrls: ['./vehicleConfiguration-wizard.component.css'],
  providers:[RPCService]
})
export class VehicleConfigurationAddWizardComponent implements OnInit {
    public myFormAddress: FormGroup;
    public myFormVehicleDetails: FormGroup;
...
public accessPoints = ['APMac'];

public busPolicyNames = ['busPolicy1'];
public portalPolicyNames = ['portalPolicy1'];
public billingPolicyNames = ['billingPolicy1'];
...
startAndEndTimeChanged(e,origin:string)
  {
    this.step2Validition=false;
      ...
      console.log(e);
      let st = this.myFormVehicleDetails.controls["startTime"].value;
      let et = this.myFormVehicleDetails.controls["endTime"].value;
      let busreg = this.myFormVehicleDetails.controls["busRegNumber"].value;
      let busroute = this.myFormVehicleDetails.controls["busRouteNumber"].value;
      let ss = this.myFormVehicleDetails.controls["ssid"].value;
            let mac = this.myFormVehicleDetails.controls["accessPointMac"].value;
      ...
      if((String(et)!="null")&&(String(et)!="")&&(String(st)!="null")&&(String(st)!=""))
      {
        ...
        if((tempEt.getTime()-tempSt.getTime())<0)
        {
          if(origin=="fromStart")
          {
            ...
            this.myFormVehicleDetails.controls["startTime"].setValue("");
            this.startTimeValue="";
          ...
          else if(origin=="fromEnd")
          {
            ...
            this.myFormVehicleDetails.controls["endTime"].setValue("");
            this.endTimeValue="";
            this.showTimeErrorMsg="block";
      ...
      st = this.myFormVehicleDetails.controls["startTime"].value;
      et = this.myFormVehicleDetails.controls["endTime"].value;
      ...
      &&(String(ss)!="null")&&(String(ss)!="")
       &&(String(mac)!="null")&&(String(mac)!=""))
      {
        this.step2Validition=true;
      }
      else
        this.step2Validition=false;
        console.log(this.myFormVehicleDetails.value);
  }
...
*/
      ...
      let ss = this.myFormVehicleDetails.controls["ssid"].value;
       let mac = this.myFormVehicleDetails.controls["accessPointMac"].value;
        ...
        else if(String(origin)=="fromBusRoute")
          {
            busroute=e.target.value;
            console.log('busRoute:'+busroute);
          }
        else if(String(origin)=="fromSSID")
          {
             ss=e.target.value;
            console.log('ssid:'+ss);
          }
        else if(String(origin)=="fromMac")
          {
             mac=e.target.value;
            console.log('mac:'+mac);
          }
    ...
    ngOnInit() {
        ...
        this.myFormAddress = this._fb.group({
            ...
            zipCode: ['', [Validators.required,CustomValidator.zipcodeValidation]],
          });

       this.myFormVehicleDetails = this._fb.group({
      ...
      this.myFormLayoutDetails = this._fb.group({
        ...
        if(this.operation=="Modify")
        {
          ...
          this.startTimeValue = String(new Date(this.data.startDate));
          this.endTimeValue = String(new Date(this.data.endDate));
                 this.step2Validition=true;
...
/*mobileNumberValidation2(e)
    {
       if (this.check(e.target.value)&&this.check(this.myFormLayoutDetails.controls.sim_1_Number.value)&&(this.allIPAddressProfiles.length>0)) {
          this.step4Validition=true;
        }
        else
          this.step4Validition = false;
...
//  console.log(this.ipAddressSelectedRowsForDeletion);
    ...
    subcribeToFormChanges() {
     ...
     /*this.myFormVehicleDetails.statusChanges.subscribe(x =>{
        console.log(x);
          if((String(x)=="VALID")||(String(x)=="valid"))
          {
            //this.step2Validition=true;
            console.log("timeRangeValid : "+this.timeRangeValid);
            if(this.timeRangeValid==true)
              this.step2Validition=true;
          }
...
let myFormVehicleDetailsData= this.myFormVehicleDetails.value;
let myFormVehicleDetails={};
myFormVehicleDetails['busRegNumber']=myFormVehicleDetailsData.busRegNumber;
myFormVehicleDetails['busRouteNumber']=myFormVehicleDetailsData.busRouteNumber;
myFormVehicleDetails['accessPointMac']=myFormVehicleDetailsData.accessPointMac;
myFormVehicleDetails['ssid']=myFormVehicleDetailsData.ssid;
myFormVehicleDetails['startDate']=myFormVehicleDetailsData.startTime;
myFormVehicleDetails['endDate']=myFormVehicleDetailsData.endTime;

let myFormLayoutDetailsData= this.myFormLayoutDetails.value;
let myFormLayoutProfile={};

     //   myFormLayoutProfile["layoutImage"]=myFormLayoutDetailsData.layoutImage;
        myFormLayoutProfile["layoutImage"]=this.imageLayoutStr;
        myFormLayoutProfile["sim_1_Number"]=myFormLayoutDetailsData.sim_1_Number;
        myFormLayoutProfile["sim_2_Number"]=myFormLayoutDetailsData.sim_2_Number;
        myFormLayoutProfile["ipAddresses"]=this.allIPAddressProfiles;

    let finalData = jQuery.extend(this.myFormAddress.value,myFormVehicleDetails,this.myFormPolicies.value,myFormLayoutProfile,{"date":new Date().toISOString()});
 ...
 let toSetAssigned='', toSetUnAssigned='', assignTo="Vehicle";
    ...
    finalData["deviceConfigNeeded"]="false";
		...
		{
      finalData["deviceConfigNeeded"]="false";
			toSetAssigned = myFormVehicleDetails["accessPointMac"];
...
//    finalData = jQuery.extend(finalData,{"uuid":this.data.uuid});
  ...
  this.clientRpc.orgRPCCall(op,"orgManagerBusConfigurationMethod",finalData)
        .subscribe(res =>{
        console.log("OUTPUT...");
        console.log(res);
        let log = new Logger();
        if(String(res.type)=="undefined")
        {
          ...
          if((res.result.status=="Success")||(res.result.Status=="Success"))
          {
            ...
            log.storeSuccessNotification("In Vehicle Configuration "+this.operation+" succeeded");
          }
          else
          {
            log.storeFailNotification("In Vehicle Configuration "+this.operation+" Failed");
