...
@Component({
  selector: 'assetAuthorizationMac-add-wizard',
  templateUrl: './assetAuthorizationMac-wizard.component.html',
  styleUrls: ['./assetAuthorizationMac-wizard.component.scss'],
  providers:[RPCService]
})
export class AssetAuthorizationMacAddWizardComponent implements OnInit {
...
locOptions: IMultiSelectOption[] = [];
    ...
    ngOnInit() {
        ...
        this.authorisedAssets = this.data.authorizedAssets;
...
addAssets(e)
{
  ...
  if((assetName.length>0)&&(mac.length>0)&&(sr.length>0))
  {
    ...
    if(String(res)=="null")
    {
      this.authorisedAssets.push({ "assetName" : assetName,
                "macAddress" : mac,
                "id" : new Date().getTime(),
                "serialNumber" : sr});
      console.log("All assets...");
      console.log(this.authorisedAssets);
  ...
  if(this.authorisedAssets.length>0)
    this.step2Validition=true;
  else
    this.step2Validition=false;
}
...
deleteSelectedAssets(e)
{
  e.preventDefault();
  console.log("Before Delete All Profiles");
  console.log(this.authorisedAssets);
  this.manualModeSelectedRowsForDeletion.forEach(ele=>{
    let index = this.authorisedAssets.findIndex(val => val.id==ele.id);
    this.authorisedAssets.splice(index,1);
})
...
onFileChange(e){
    ...
    catch(e)
    {
      ...
      this.myFormCSVMode.controls.autherizedAssetsCSVFile.setErrors({"selectCSVFile":true});
      this.clearSelectedFileContent();
...
processFile(e){
    ...
    try{
      var assetArray=this.fileStr.split("\n");
      if(assetArray.length>0)
      {
          let docHeader= assetArray[0];
          ...
          if(docHeaderColumns.length>0)
          {
            ...
            {
              if(docHeaderColumns[i].indexOf("Asset Name")>=0)
              {
                assetNameIndex=i;
                anIndexFound=true;
              }
          ...
          console.log("asset : "+assetNameIndex+", MAC : "+macIndex+", SR : "+serialNumberIndex);
          if(anIndexFound&&macIndexFound&&srIndexFound)
          {
              this.authorisedAssets=[];
              for(let j=1;j<assetArray.length;j++)
              {
                let currCols= assetArray[j].split(",");
          ...
          console.log(this.authorisedAssets);
          this.myFormCSVMode.controls.autherizedAssetsCSVFile.setErrors(null);
          //this.authorisedAssets=[this.parsedFile];
      ...
      this.myFormCSVMode.controls.autherizedAssetsCSVFile.setErrors({"invalidCSVFile":true});
    }
  }
 ...
 let locsFullData = this.locationData.filter(val => formData.locations.includes(val.uuid));
    ...
    finalData["authorizedAssets"]=this.authorisedAssets;
  ...
  this.clientRpc.orgRPCCall(op,"orgManagerAuthorisedAssetsMACMethod",finalData)
        .subscribe(res =>{
        console.log("OUTPUT...");
        console.log(res);
        let log = new Logger();
        if(String(res.type)=="undefined")
        {
          ...
          if((res.result.status=="Success")||(res.result.Status=="Success"))
          {
            ...
            log.storeSuccessNotification("In AP Authorization MAC "+this.operation+" succeeded");
          }
          else
          {
            log.storeFailNotification("In AP Authorization MAC "+this.operation+" Failed");
