...
@Component({
  selector: 'db-asset-table',
  templateUrl: './dashboardAssetTable.component.html',
  providers:	[CollectionTableDataCollectorService],
  encapsulation: ViewEncapsulation.None
})
export class DashboardAssetTable implements OnInit {
...
let deviceID = this.deviceID;
    ...
    this.queryResponse.forEach(element => {
      assetUsedData[element.apMAC]=element.value;
    });
    let docs=[];
        for(var len=0;len<this.assetsData.length;len++)
        {
            ...
            let usedData=0;
            if(assetUsedData[ap.macAddress]!=undefined)
              usedData = assetUsedData[ap.macAddress];
            var ext="";
